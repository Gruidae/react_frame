...
const config = {
  title: '海绵拍',                          // Your website title
  url: 'https://pai.bigins.cn',             // Your website URL
...
tasks.set('start', () => {
  ...
  return run('clean').then(() => new Promise((resolve) => {
    ...
    compiler.plugin('done', (stats) => {
      ...
      const bundle = stats.compilation.chunks
        .find(x => x.name === 'main').files[0];
      const bundleCss = stats.compilation.chunks
        .find(x => x.name === 'main').files
        .filter(item => item.indexOf('styles') > -1)[0];
      const template = fs.readFileSync('./public/index.ejs', 'utf8');
      const render = ejs.compile(template, { filename: './public/index.ejs' });
      const output = render({
        debug: true,
      ...
      if (count === 1) {
        bs.init({
          ...
          snippetOptions: {
            ignorePaths: '*',
            blacklist: 'src/containers/profile/RegisterPage.jsx',
          },
        }, resolve);
      }
    });
